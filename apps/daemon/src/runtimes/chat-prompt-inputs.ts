...
export function formatDesignFilesWorkspaceHint(
  ...
  const lines = [
    ...
    'Multi-page project connectivity (CRITICAL): When creating or adding a new page to a project with existing HTML files (e.g. creating `register.html` when `login.html` exists, or adding `dashboard.html`/`landing.html`), inspect existing files to ensure all pages share the same visual brand tokens, link the new page to existing pages with relative links, and update existing pages to replace placeholder links (`href="#"`) with the newly created page path so all pages in the prototype connect seamlessly.',
