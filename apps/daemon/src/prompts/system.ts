...
function renderMetadataBlock(
  ...
  if (!factsOnly && (metadata.kind === 'prototype' || metadata.kind === 'template' || metadata.kind === 'other')) {
    lines.push(
      '- **in-place revisions vs. new page creation (CRITICAL)**: When modifying or revising an existing page, always edit the existing file directly in-place with the same filename/identifier (never create `login-v2.html`, `2.html`, etc.) and preserve design continuity (layout, typography, fonts, color tokens, CSS classes). When the user asks for a new distinct page/screen (e.g. creating a landing page after a login page), create a NEW file with its own appropriate identifier (e.g. `landing.html`) without overwriting existing pages, maintain brand design consistency, and connect them with relative links (e.g. `<a href="login.html">Log In</a>`).',
    );
    lines.push(
