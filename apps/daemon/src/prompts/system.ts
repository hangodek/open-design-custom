...
function renderMetadataBlock(
  ...
  if (!factsOnly && (metadata.kind === 'prototype' || metadata.kind === 'template' || metadata.kind === 'other')) {
    lines.push(
      '- **in-place artifact & design continuity rule (CRITICAL)**: Always modify and refine existing files in-place. Never create versioned copies (do not create `login-v2.html`, `login-html2.html`, `2.html`, etc.) when responding to changes or revisions. During revisions, preserve the established layout structure, visual hierarchy, typography, fonts, color tokens, and CSS styles of the existing design — make surgical, focused updates to requested elements rather than rewriting untouched sections from scratch. The live preview, checkpoint rollback system, and inspect tools rely on preserving the canonical filename and design continuity across all revisions.',
    );
    lines.push(
